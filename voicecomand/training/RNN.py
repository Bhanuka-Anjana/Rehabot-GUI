...
class TrainingLog(object):
    def __init__(
        self,
        training_args=None,
    ):  # arguments in training    #  MAX_EPOCH = 1000,
...
class RNN(nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, num_classes, device, classes=None
    ):
    ...
    def predict_probabilities(self, x):
        """
        Given the feature x of an audio sample, compute the probability of classified as each class.
        :param x: x {np.array}: features of a sample audio.
                Shape = L*N, where
                L is length of the audio sequence,
                N is feature dimension.
        :return: probs {np.array}: Probabilities.
        """
    ...
    def predict(self, x):
        ...
        :param x: {np.array}: features of a sample audio.
                Shape = L*N, where
                L is length of the audio sequence,
                N is feature dimension.
    ...
    def predict_audio_label(self, audio):
        ...
        if not self.classes:
            raise RuntimeError(
                "Classes names are not set. Don't know what audio label is"
            )
...
def load_weights(model, weights):
    ...
    for i, (name, param) in enumerate(weights.items()):
        ...
        if model_shape != param.shape:
            print(
                "\nWarning: Size of {} layer is different between model and weights. Not copy parameters.".format(
                    name
                )
            )
            print(
                "\tModel shape = {}, weights' shape = {}.".format(
...
def create_RNN_model(args, load_weight_from=None):
    """
    :param args: classes,classes_txt,num_classes,save_log_to,save_fig_to,device,input_size,hidden_size,num_layers
    :param load_weight_from: if exsit model, load
    :return: model
    """
    ...
    if hasattr(args, "classes"):
        ...
        classes = support_functions.read_list(args.classes_txt)
    else:
    ...
    device = args.device
    model = RNN(
        args.input_size, args.hidden_size, args.num_layers, args.num_classes, device
    ).to(device)
    ...
    for epoch in range(1, 1 + args.num_epochs):
        ...
        for i, (featuress, labels) in enumerate(train_loader):
            ...
            if (i + 1) % 50 == 0 or (i + 1) == len(train_loader):
                print(
                    "Epoch [{}/{}], Step [{}/{}], Loss = {:.4f}, Train accuracy = {:.2f}".format(
                        epoch,
                        args.num_epochs,
                        i + 1,
                        total_step,
        ...
        if (epoch) % 1 == 0 or (epoch) == args.num_epochs:
            ...
            if args.save_model_to:
                name_to_save = (
                    args.save_model_to + "/" + "{:03d}".format(epoch) + ".ckpt"
                )
...
def evaluate_model(model, eval_loader, num_to_eval=-1):
    ...
    print(
        "  Evaluate on eval or test dataset with {} samples: Accuracy = {}%".format(
            i + 1, 100 * eval_accu
        )
    )
    return eval_accu
